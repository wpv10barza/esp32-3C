...
// A 10-column grid makes the hit areas deterministic and easy to validate.
// The keyboard is intentionally confined to the lower 264 px of the panel.
constexpr int kKeyboardX = 2;
constexpr int kKeyboardY = 216;
constexpr int kKeyboardWidth = 476;
constexpr int kKeyboardHeight = 256;
constexpr int kColumns = 10;
constexpr int kRows = 5;
constexpr int kColumnGap = 4;
constexpr int kRowGap = 4;
constexpr int kKeyWidth = 44;
constexpr int kKeyHeight = 48;

static_assert(kKeyboardX >= 0 && kKeyboardY >= 0,
              "keyboard origin must be inside screen");
...
static_assert(kKeyboardY + kKeyboardHeight <= kScreenHeight,
              "keyboard must stay inside 480px screen");
static_assert(kColumns * kKeyWidth + (kColumns - 1) * kColumnGap <= kKeyboardWidth,
              "keyboard grid must fit inside keyboard frame");
...
// This means touching x == right belongs to the next key (or to no key at
// the outer edge), preventing two keys from responding to one touch.
struct KeyRect {
  int16_t left = 0;
  int16_t top = 0;
  int16_t right = 0;
