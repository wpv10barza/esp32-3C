...
constexpr uint16_t kFieldFill = 0x0B1822;
...
void CommandInput::begin(Arduino_GFX* display) {
  display_ = display;
}
...
void CommandInput::moveLeft() {
  if (cursor_ > 0) --cursor_;
}

void CommandInput::moveRight() {
  if (cursor_ < buffer_.length()) ++cursor_;
}

void CommandInput::moveHome() {
  cursor_ = 0;
}

void CommandInput::moveEnd() {
  cursor_ = buffer_.length();
}

void CommandInput::drawKey(int x, int y, int width, const char* label, uint8_t textSize) {
  if (!display_) return;
  const uint16_t fill = (strcmp(label, "⌫") == 0 || strcmp(label, "CLR") == 0) ? kKeySpecial : kKeyFill;
...
void CommandInput::drawField() {
  ...
  const size_t start = buffer_.length() > visibleChars && cursor_ > visibleChars
                         ? cursor_ - visibleChars
                         : 0;
  const String visible = buffer_.substring(start);
  display_->setCursor(kFieldX + 9, kFieldY + 12);
  display_->print(visible.substring(0, visibleChars));
...
void CommandInput::draw() {
  ...
  for (int row = 0; row < 4; ++row) {
    ...
    for (int index = 0; index < 10; ++index) {
      ...
      drawKey(kKeyboardX + index * (kKeyW + kKeyGap), y, kKeyW, label, 1);
    }
  }

  const int specialY = kKeyboardY + 4 * (kKeyH + kKeyGap);
  drawKey(kKeyboardX, specialY, 182, "ESPACIO", 1);
  drawKey(kKeyboardX + 184, specialY, 90, "⌫", 1);
  drawKey(kKeyboardX + 276, specialY, 90, "CLR", 1);
  drawKey(kKeyboardX + 368, specialY, 46, "<-", 1);
  drawKey(kKeyboardX + 418, specialY, 46, "->", 1);
}

bool CommandInput::handleTouch(uint16_t x, uint16_t y) {
  if (!active_) return false;

  if (y >= kFieldY && y < kFieldY + kFieldH &&
      x >= kFieldX && x < kFieldX + kFieldW) {
