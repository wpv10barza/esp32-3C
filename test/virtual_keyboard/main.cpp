...
void assertNoOverlaps(KeyboardMode mode) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    for (size_t j = i + 1; j < count; ++j) assert(!virtual_keyboard::rectanglesOverlap(keys[i].rect, keys[j].rect));
  }
}
...
void assertControlKeysHit(KeyboardMode mode) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    assert(virtual_keyboard::hitTestIndex(mode, x, y) == static_cast<int>(i));
  ...
  assert(foundToggle && foundSpace && foundEnter);
}
